...
| `gs-ia-head-innovation-vp.html` | **`Yasir_Malik_Resume_GS_IA_HeadInnovation_VP.pdf` — Goldman Sachs Internal Audit, Head of Innovation, NY.** Plain executive cut, unbranded (Goldman OBA disclosure). **The best-matched role on the board:** the posting asks for someone to define an audit AI roadmap, ship analytics products on it, and build the framework for auditing the firm's own AI — he did all three at Citi Internal Audit. Leads with that. Claims no R and no Snowflake, which the posting names and the record does not hold. Built 24 Sep 2026. | headless Chromium, below |
